            ...
            <div class="bg-white rounded-lg shadow-xl w-full max-w-lg" @click.away="open = false">
                ...
                <form wire:submit="store">
                    <div class="p-6 space-y-4">
                        <div>
                            ...
                            <input type="text" id="name"
                                class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-ifnmg-green focus:border-ifnmg-green @error('name') border-red-500 @enderror"
                                wire:model="name">
                            @error('name')
                        ...
                        <div>
                            ...
                            <input type="email" id="email"
                                class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-ifnmg-green focus:border-ifnmg-green @error('email') border-red-500 @enderror"
                                wire:model="email">
                            @error('email')
                                <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                            @enderror
                        ...
                        <div>
                            ...
                            <select id="role"
                                class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-ifnmg-green focus:border-ifnmg-green @error('role') border-red-500 @enderror"
                        ...
                        @if ($role === 'fiscal')
                            <div class="mt-4">
                                <x-input-label for="fiscal_type" value="Especialidade do Fiscal" />
                                ...
                                <x-input-error for="fiscal_type" class="mt-2" />
                            </div>
                        @endif
                        <div>
                            <label for="password" class="block text-sm font-medium text-gray-700">Senha</label>
                            <input type="password" id="password"
                                class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-ifnmg-green focus:border-ifnmg-green @error('password') border-red-500 @enderror"
                                wire:model="password">
                            <p class="text-xs text-gray-500 mt-1">
                                {{ $userId ? 'Deixe em branco para não alterar a senha.' : '' }}</p>
                            @error('password')
                                <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                            @enderror
                        </div>
                    ...
                    <div class="px-6 py-4 bg-gray-50 text-right space-x-2">
                        ...
                        <button type="submit"
                            class="px-4 py-2 bg-blue-600 text-white rounded-md hover:bg-blue-700">{{ $userId ? 'Atualizar Usuário' : 'Salvar Usuário' }}</button>
                    </div>
                </form>
            </div>
        </div>
            ...
            <div class="bg-white rounded-lg shadow-xl w-full max-w-md" @click.away="open = false">
                <div class="p-6">
                    ...
                    <p class="mt-2 text-sm text-gray-600">Você tem certeza que deseja excluir o usuário
                        <strong>{{ $userNameToDelete }}</strong>?</p>
