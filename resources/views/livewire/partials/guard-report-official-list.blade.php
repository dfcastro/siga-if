<div>
    <table class="w-full text-sm text-left text-gray-500">
        ...
        <tbody class="space-y-4 sm:space-y-0">
            @forelse ($vehiclesWithOfficialTrips as $item)
                {{-- Verifica se o item e o veículo não são nulos antes de usar --}}
                @if (isset($item['vehicle']))
                    <tr
                        ...
                        <td
                            ...
                            <span class="text-right">{{ $item['vehicle']->model }}
                                <span class="block text-xs">{{ $item['vehicle']->license_plate }}</span>
                            </span>
                        </td>
                        <td
    ...
    {{-- Links da Paginação (já estava correto) --}}
    <div class="mt-4">
        @if ($officialTrips)
            {{ $officialTrips->links() }}
        @endif
